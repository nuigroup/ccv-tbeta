		...
		if(fiducials[i].id!=INVALID_FIDUCIAL_ID){
			...
			if (existing_fiducial!=NULL) {//if found
				...
				//added by Stefan Schlupek
					...
					//Beware of a list: as you push_back we create a copy so addFiducial.initLastFrame() doesn't work :-(
					/*we have to set the LastFrame as in the below iteration we check for isAlive and set current.xpos	= last.xpos
						so otherwise we get in the ofxNCoreVision::prepareFiducialPositions() only -100,-100 for the position.
					*/
					fiducialsList.back().initLastFrame();
