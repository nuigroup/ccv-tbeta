...
void BlobTracker::passInCamCalibration(ofCvCameraCalibration* camCalibrater){
	camCalibrate = camCalibrater;
}
		...
		else //living, so update it's data
		{
			for(int j=0; j<newBlobs->nBlobs; j++)
			{
				if(trackedBlobs[i].id==newBlobs->blobs[j].id)
				{
					...
					//printf("BlobID:%i,D(%f, %f)\n",trackedBlobs[i].id, trackedBlobs[i].D.x, trackedBlobs[i].D.y);
					printf("Blob.centroid:(%f,%f)\n",trackedBlobs[i].centroid.x, trackedBlobs[i].centroid.y);
					//printf("lastCentroid:(%f,%f)\n",trackedBlobs[i].lastCentroid.x, trackedBlobs[i].lastCentroid.y);
...
//					}
					...
					trackedBlobs[i].maccel = sqrtf((tD.x* tD.x)+(tD.y*tD.y))/(ofGetElapsedTimef() - trackedBlobs[i].lastTimeTimeWasChecked)/1000.0f;
					
					
				//printf("ofGetElapsedTimef() %f,trackedBlobs[i].lastTimeTimeWasChecked %f)\n", ofGetElapsedTimef(), trackedBlobs[i].lastTimeTimeWasChecked);
				//trackedBlobs[i].lastTimeTimeWasChecked = ofGetElapsedTimeMillis();!!!!!!!!!!!!!!!!!!!!!!!!!	
				//calculate the age
					trackedBlobs[i].age = ofGetElapsedTimef() - trackedBlobs[i].downTime;
					//printf("maccel %f\n", trackedBlobs[i].maccel);
					...
					//printf("blob.sitting: %f\n", trackedBlobs[i].sitting );
					//printf("blob.downTime: %f\n", trackedBlobs[i].downTime );
					...
					if(trackedBlobs[i].sitting > 1.0f){
						...
						//Calibrated dx/dy
						//changed by Stefan Schlupek: was a big  integer/float/ milliseconds  mess
						...
						/*
						printf("Tracking: Blob.D: %f:%f\n", TouchEvents.messenger.D.x,TouchEvents.messenger.D.y );
						printf("TouchEvents.messenger.centroid: %f:%f\n", TouchEvents.messenger.centroid.x,TouchEvents.messenger.lastCentroid.y );
						
						printf("Tracking: ofGetElapsedTimeMillis:lastTimeTimeWasChecked %i::%i\n",ofGetElapsedTimeMillis(),TouchEvents.messenger.lastTimeTimeWasChecked );
						printf("Tracking: ofGetElapsedTimef: %f\n",ofGetElapsedTimef());
						*/
						
						TouchEvents.messenger.lastTimeTimeWasChecked = ofGetElapsedTimef();
						
						//calibrated accelleration changed by Stefan Schlupek
						ofPoint tD2 = TouchEvents.messenger.D;
						TouchEvents.messenger.maccel = sqrtf( (tD2.x* tD2.x)+(tD2.y*tD2.y) )/(ofGetElapsedTimef() - trackedBlobs[i].lastTimeTimeWasChecked)/1000.0f;						
