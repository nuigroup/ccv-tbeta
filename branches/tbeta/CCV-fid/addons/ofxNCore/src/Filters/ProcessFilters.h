 ...
 ****************************************************************/
     void applyCPUFilters(CPUImageFilter& img){
		...
		// stefan Schlupek : this line causes CCV to grab a background on startup. have to be removed for loading backround function. Hope that there is no new bug by this
        //recapature the background until image/camera is fully exposed
		if(backgroundMode =="AUTO" &&(ofGetElapsedTimeMillis() - exposureStartTime) < CAMERA_EXPOSURE_TIME ) bLearnBackground = true ;

        //Capture full background
        if (bLearnBackground == true){
			bLearnBackground = false;
			setCPUBackgroundImage(img);
		}
 ...
 ****************************************************************/
    ...
    void drawGPU()
    {
	 ...
	 void drawGPU_fiducial()
	 {
		  gpuReadBackImageGS.draw(720,30,320,240);
	 }

	 void draw_fiducial(){
		 grayDiff.draw( 720,30,320,240);
	 }
	void draw(float x,float y,float width,float height)
    {
       grayDiff.draw(x, y, width, height);
	   // grayDiff.draw(385, 30, 320, 240);
		/*
		 grayImg.draw(40, 30, 320, 240);
        grayDiff.draw(385, 30, 320, 240);
	...
	void setCPUBackgroundImage(ofxCvGrayscaleImage &img){
			floatBgImg = img;
			//grayBg = floatBgImg;  // not yet implemented
			cvConvertScale( floatBgImg.getCvImage(), grayBg.getCvImage(), 255.0f/65535.0f, 0 );       
			grayBg.flagImageChanged();
			printf("setCPUBackgroundImage\n");
	}
