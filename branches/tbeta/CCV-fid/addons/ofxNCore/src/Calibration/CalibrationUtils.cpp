...
void CalibrationUtils::init_UDP_calibrate_connection(string host,int port){

	udpConnection.Create();
	//udpConnection.Connect("127.0.0.1",11999);
	//udpConnection.Connect(calibration_udp_host,calibration_udp_port);
...
void CalibrationUtils::loadXMLSettings(){
	...
	//---------added by Stefan Schlupek-----------
	//at this moment we don't need the undistortion but for future releases...
	
	CvSize csize;
	 csize = cvSize( _camWidth,_camHeight );
	printf("_camWidth:%i _camHeight:%i \n",_camWidth ,_camHeight);
    camCalib.allocate( csize, 7, 7 );//important: allocate Memory/Image for the maps	
	camCalib.loadData();
	//generate Map:
	//With this map we could map the Blob Points to an undistortet position,without the need to undistort the Camera Image every frame.
	if(camCalib.intrinsic_matrix != 0 && camCalib.distortion_coeffs !=0){
		camCalib.createUndistortMap();
	}
		/**/
	//---------added by Stefan Schlupek-----------
}
	...
	for(int y = 0; y < _camHeight; y++)
	{
		for(int x = 0; x < _camWidth; x++)
		{
			...
			cameraToScreenMap[p] = vector2df(transformedX, transformedY);
				//printf("cameraToScreenMap.pt.%i:%f:%f\n", p,transformedX,transformedY);
			p++;
		}
	}
}
...
void CalibrationUtils::cameraToScreenPosition(float &x, float &y)
{
	...
	//legacy code but it could be usefull in the future
	//---------added by Stefan Schlupek-----------
	//std::cout << "pos: x:" << x << ", y: "<< y << std::endl;
	camCalib.undistortPoints2(x,y);
	//std::cout << "pos: x:" << x << ", y: "<< y << std::endl;
	x /= _camWidth ;
	y /= _camHeight;
	//std::cout << "pos2: x:" << x << ", y: "<< y << std::endl;
	//---------added by Stefan Schlupek-----------
	return;
}
