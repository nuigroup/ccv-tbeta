...
void ofVideoGrabber::grabFrame(){
	...
	#ifdef OF_VIDEO_CAPTURE_DIRECTSHOW
	//---------------------------------
//printf("ofVideoGrabber.grabFrame.bGrabberInited: %s ",bGrabberInited ? "true" : "false");
//printf("ofVideoGrabber.grabFrame.VI.isFrameNew(device): %s \n",VI.isFrameNew(device) ? "true" : "false");
		if (bGrabberInited == true){
			bIsFrameNew = false;
