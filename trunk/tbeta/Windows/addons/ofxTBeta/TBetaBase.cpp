...
void TBetaBase::drawToScreen()
{
    ...
    if (bFastMode)
    {
        ...
        if (bcamera)
        {
            sidebarTXT.drawString("Camera [ms]:            " + ofToString(fps,0),10, ofGetHeight() - 29 );
        }
        else
        {
        ...
        sidebarTXT.drawString("TUIO:" + ofToString(differenceTime,0),10, ofGetHeight() - 9 );
        ...
        if (bTUIOMode)
        {
            ...
            ofSetColor(0x00FF00);
            ofFill();
            ofCircle(ofGetWidth() - 16 , ofGetHeight() - 10, 5);
            ofNoFill();
        }
    }
