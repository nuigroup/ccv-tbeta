			...
			for(int j=0; j<newBlobs->nBlobs; j++)
			{
				if(trackedBlobs[i].id==newBlobs->blobs[j].id)
				{
                    ...
                    if(abs((int)trackedBlobs[i].lastCentroid.x - (int)trackedBlobs[i].centroid.x) < 3 && 
						...
						if(trackedBlobs[i].sitting != -1) trackedBlobs[i].sitting = 1;
					} else {
					...
					on down:  orginX = blobx; aliveTime = ofgetelaspedtime;
					
					if(currentTime - aliveTime > holdThreshold && orginX/orginY - currentX/Y < 10) { do hold event }
					
					 */
					
					if(trackedBlobs[i].age > HOLDTHRESHOLD && trackedBlobs[i].sitting == 1) {
						TouchEvents.messenger = trackedBlobs[i];
						TouchEvents.RAWmessenger = trackedBlobs[i];
